...
`create_sample_data` creates representative objects used by URL discovery tests, including pages, images, documents, redirects, snippets, settings, chooser-backed models, form pages, routable event pages, and a secondary non-default Wagtail `Site` (`sub.localhost`) for multisite/subdomain coverage.
