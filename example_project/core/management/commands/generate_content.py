...
from example_project.for_snippets.models import (
    ExampleSnippetModel,
    ExampleSnippetViewSetModel,
)
from example_project.core.models import (
    ExamplePageModelBasic,
...
class Command(BaseCommand):
        ...
        for i, color in enumerate(colors):
            ...
            if not Image.objects.filter(title=image_title).exists():
                ...
                image_file = ContentFile(
                    img_io.read(), name=f"example_image_{i + 1}.jpg"
                )
    ...
    def generate_example_documents(self):
        ...
        for i, (ext, content) in enumerate(document_types):
            ...
            if not Document.objects.filter(title=document_title).exists():
                ...
                doc_file = ContentFile(
                    doc_content, name=f"example_document_{i + 1}.{ext}"
                )
    ...
    def generate_example_pages(self):
        ...
        if not example_images.exists():
            self.stdout.write(
                "No images found to associate with example pages. Please create some images first."
            )
            return

        root_page = Page.objects.get(
            id=3
        )  # ID 3 should be OK here as it's created by the initial migration

        for i in range(5):
            # Create basic page type
            if not ExamplePageModelBasic.objects.filter(
            ...
            if not ExamplePageModelStandard.objects.filter(
    ...
    def generate_example_form_pages(self):
        ...
        for i in range(5):
            if not ExampleFormPage.objects.filter(
                ...
                for j in range(5):
                    FormSubmission.objects.create(
                        page=form_page,
                        form_data={
                            "name": f"Example User {j + 1}"
                        },  # JSON field format for form data
                    )
    ...
    def generate_example_collections(self):
        ...
        for i in range(5):
            ...
            existing_collection = (
                root_collection.get_children().filter(name=collection_name).first()
            )
        ...
        for i, config in enumerate(redirect_configs):
            ...
            if not Redirect.objects.filter(old_path=old_path).exists():
                ...
                if config["redirect_type"] == "page":
                    redirect_kwargs["redirect_page"] = example_pages[
                        i % len(example_pages)
                    ]
                    target_description = (
                        f"page '{redirect_kwargs['redirect_page'].title}'"
                    )
                else:
                ...
                self.stdout.write(
                    f"Created {redirect_type} redirect: {old_path} → {target_description}"
                )
