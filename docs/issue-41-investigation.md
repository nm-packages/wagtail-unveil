...
In multisite setups, Wagtail returns absolute URLs (for example `http://sub.localhost:8000/about/`) for pages on non-default sites.
...
1. Configure two Wagtail `Site` records:
   - default: `localhost`
   - non-default: `sub.localhost:8000`
...
- Add sandbox sample data support for a non-default `sub.localhost:8000` site fixture.
