(() => {
  ...
  (() => {
    ...
    var platformPypiResults = /* @__PURE__ */ new Map();
    ...
    function setPlatformMarkdownButtonState(options) {
      ...
      var isLoading = options && typeof options.isLoading === "boolean" ? options.isLoading : false;
      var label = options && typeof options.label === "string" ? options.label : "Render Markdown Report";
      if (!button) {
        return;
      }
      button.disabled = isLoading;
    ...
    function resetPlatformClientState() {
      ...
      platformPypiResults = /* @__PURE__ */ new Map();
    ...
    function setPlatformMarkdownCopyButtonState(options) {
      ...
      var isDisabled = options && typeof options.isDisabled === "boolean" ? options.isDisabled : false;
      var label = options && typeof options.label === "string" ? options.label : "Copy Markdown";
      if (!button) {
        return;
      }
    ...
    function copyPlatformMarkdownToClipboard() {
      ...
      if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
        return navigator.clipboard.writeText(output.value).then(() => true).catch(() => fallbackCopyPlatformMarkdown(output));
      }
      ...
      document.querySelectorAll("#platform-packages-body tr[data-package-lookup-name]").forEach((row) => {
        ...
        if (result.status === "ok") {
          ...
          setPypiLookupCell(row, {
            ...
            toneClass: display.marker === "Latest" ? "platform-pypi-success" : "platform-pypi-warning",
    ...
    function buildPlatformMarkdownReport() {
      var runtime = platformSnapshotState && platformSnapshotState.runtime || {};
      var source = platformSnapshotState && platformSnapshotState.source || {};
      var warnings = platformSnapshotState && platformSnapshotState.warnings || [];
      var packages = platformSnapshotState && platformSnapshotState.packages || [];
      var metadata = platformSnapshotState && platformSnapshotState.metadata || {};
      var installedCount = packages.filter((item) => item.is_installed).length;
      ...
      return [
        "# Platform Report",
        "",
        "Packages: " + packages.length + " total, " + installedCount + " installed, " + (packages.length - installedCount) + " missing, " + warnings.length + " warnings.",
        "",
        "## Runtime",
    ...
    function renderPlatformMarkdownReport() {
      ...
      return ensurePlatformPyPiVersionsLoaded().then(() => {
    ...
    report.data = {
      ...
      bindPlatformMarkdownReport,
      bindPlatformPyPiLookup,
      buildPlatformMarkdownReport,
      ensurePlatformPyPiVersionsLoaded,
      loadReportData,
      lookupPlatformPyPiVersions,
      renderPlatformMarkdownReport
    };
