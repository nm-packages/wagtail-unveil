...
Built-in hook resolvers currently cover Wagtail form submissions, an explicit safe allowlist of single-parameter `wagtailadmin_pages` routes, a compatibility-aware `add_subpage` parent-page fallback, workflow usage routes, and GET-safe workflow task routes.
